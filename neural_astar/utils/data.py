...
def create_mario_dataloader(
    ...
    dataset = MarioDataset(dirname, split)
    return data.DataLoader(
        dataset, batch_size=batch_size, shuffle=shuffle, num_workers=0
    )
...
class MarioDataset(data.Dataset):
    def __init__(
        ...
        self.map_designs = np.load(f"{dirname}/{split}_maps.npy")
        self.paths = np.load(f"{dirname}/{split}_shortest_paths.npy").astype(np.float32)

    def __getitem__(self, index: int):
        map_design = get_img_from_level(np.argmax(self.map_designs[index], axis=0)).transpose(2, 0, 1)
        opt_traj = self.paths[index][np.newaxis]
        # Calculate the start and goal maps
        for i in range(14):
            column = opt_traj[0, :, i]
            if 1 not in column:
                continue
            ind = np.where(column==1)[0][-1]
            start_coord = (ind, i)
            break
        for i in reversed(range(14)):
            ...
            if 1 not in column:
                continue
            ind = np.where(column==1)[0][0]
            goal_coord = (ind, i)
            break
        start_map = np.zeros_like(opt_traj)
        goal_map = np.zeros_like(opt_traj)
        start_map[:, start_coord[0], start_coord[1]] = 1
        goal_map[:, goal_coord[0], goal_coord[1]] = 1
